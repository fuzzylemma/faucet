...
contract Faucet {
    ...
    // token => faucet owner
    mapping(address => address) internal plumber;
    // token => drip size 
    mapping(address => uint256) internal dripSize;
    // token => time between faucet requests;
    //      a value of 0 implies faucet is closed 
    mapping(address => uint256) internal dripFrequency;
    // token => account => last request
    mapping(address => mapping(address => uint256)) internal lastSip;
    ...
    function destroyFaucet(address token) external onlyPlumber(token) {
        ...
        IERC20(token).approve(token, balance);
